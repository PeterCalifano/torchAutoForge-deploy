...
std::string getMexFilePath() {
    HMODULE hModule = NULL;
    // This tells Windows to return the handle to *this* DLL (the MEX file)
    GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | 
                      GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                      (LPCSTR)&getMexFilePath, // any function in the MEX file
                      &hModule);

    char path[MAX_PATH];
    DWORD len = GetModuleFileNameA(hModule, path, MAX_PATH);
    if (len == 0 || len == MAX_PATH)
        throw std::runtime_error("Failed to get MEX file path.");
    return std::string(path, len);
}

std::string getMexFileDir() {
    std::string full_path = getMexFilePath();
...
void unpackOutputsToPlhs(const uint8_t* output_with_header, mxArray** plhs) {
    ...
    for (size_t i = 0; i < num_outputs; ++i) {
        ...
        size_t elem_count = 1;
        for (mwSize d : dims) elem_count *= d;
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    // Launch process
     // Pipe setup for stdout and stderr
    SECURITY_ATTRIBUTES sa = { sizeof(SECURITY_ATTRIBUTES), NULL, TRUE };
    HANDLE hRead, hWrite;
    if (!CreatePipe(&hRead, &hWrite, &sa, 0)) {
        mexErrMsgTxt("Failed to create pipe.");
    }
    SetHandleInformation(hRead, HANDLE_FLAG_INHERIT, 0);  // prevent child from inheriting read end
    ...
    CloseHandle(hWrite);  // parent doesn't need write end anymore

    if (!success) {
        CloseHandle(hRead);
        mexErrMsgTxt("Failed to start inference process.");
    }

    // Read from pipe and print to MATLAB
    char buffer[256];
    DWORD bytesRead;
    while (ReadFile(hRead, buffer, sizeof(buffer) - 1, &bytesRead, NULL) && bytesRead > 0) {
        buffer[bytesRead] = '\0';
        mexPrintf("%s", buffer);
        mexEvalString("drawnow");  // Flush MATLAB UI output
    }
    ...
    if (!GetExitCodeProcess(pi.hProcess, &exitCode)) {
        ...
        mexErrMsgTxt("Failed to get process exit code.");
    }
    ...
    if (exitCode != 0) {
        ...
        CloseHandle(pi.hThread);
        std::remove(input_file_str.c_str());
        mexErrMsgTxt("Inference process failed with non-zero exit code.");
    }  
